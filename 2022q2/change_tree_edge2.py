...
for _ in range(t):
        ...
        while que.empty() == False:
            ...
            for dest in graph[current]:
                ...
                if dist[start][dest] == -1 or nextDist <= dist[start][dest]:
                    ...
                    #print("found!: from", start, "to", dest, que.queue)
    ...
    for mi in range(m):
        # change dist
        dist_change = [[-1 for i in range(n)] for j in range(n)]
        for nii in range(n):
            dist_change[nii][nii] = 0
        # change graph
        ...
        # find value
        for ni in range(n):
            dijkstra(ni, dist_change)
        #print(dist_change)
        #print(sum(sum(dist_change, [])))
        tmp = sum(sum(dist_change, []))
