...
public class Problem_4_timeout{
	static class Road {
		int start, end, C, T;
		int lastCost = 0;
		int lastP = 0;
	...
	public static void main(String args[]){
		...
		ArrayList<Road> course = new ArrayList<Road>(); 
		search(1, course);
...
*/		if(node == n) {
			...
			if(max_p > 0) {
				int sum = 0;
				for(int i=0;i<course.size();i++) {
					sum += course.get(i).cost(max_p + 1);
					if(sum > k) {
						//course.forEach((e) -> e.print());System.out.println(" : executed!");
						return 0;
					}
				}
			}
